...
你用 GuoLaiCode 跑了一个稍微复杂的任务，比如重构一个模块。Agent 先读了十几个文件了解代码结构，然后改了五六个文件，中间跑了好几次测试和编译。一切顺利。
...
<persisted-output>
输出太大（80KB），完整内容已保存到：
.guolaicode/session/tool-results/toolu_abc123.txt
...
这里有一个设计前提需要先交代：GuoLaiCode 不会修改原始对话记录，conversation.history 里存的永远是工具返回的原文。每轮发 API 请求前，系统都会遍历整段对话，把替换决策重新应用上去，生成一份新的消息列表发给 Anthropic。这意味着同一个工具结果，每一轮都会被重新过一遍「该不该替换」的判断。如果某一轮判定结果和上一轮不同，对话前缀就变了。
...
GuoLaiCode 采用的解决方案叫「决策冻结」，这也是 Claude Code 使用的策略。系统维护一个 `ContentReplacementState` ，记录每个 `tool_use_id` 的替换决策。规则很简单：
...
为什么要留 13,000 的安全余量？自动压缩的检查点是每轮循环开始时、调 API 之前。检查通过后，模型回复、执行工具、产生新的 tool result，这些新增内容在下一次检查之前是没有被覆盖到的。而且模型可以在一次回复中发起多个并行工具调用，一轮下来可能同时新增好几个 tool result。13,000 的 buffer 就是给这个「两次检查之间的盲区」留的余量。 13K 是一个经验值，不是精确推导出来的，你自己做 Agent 时可以根据实际场景设一个合适的数。它也不需要精确，因为即使超了也不会崩，后面「紧急压缩」会兜底：API 返回 prompt\_too\_long 时，Agent 自动触发一次压缩再重试，不会直接瘫掉。

这里还有一个容易被忽略的细节， 关系到「已用了多少 token」这个数到底准不准。GuoLaiCode 并不是把整段对话拿字符数粗估一遍，那样误差会随对话变长越滚越大。 它的做法是锚定上一次真实的 API usage（input、cache\_read、cache\_creation、output 这几项加起来），那是 Anthropic 返回的精确值；只有这次 usage 之后新增的那一小段消息，才用字符数除以 3.5 到 4 来增量估算。也就是说，整段历史的绝大部分都是精确的，估算误差只发生在最后那一小段增量上。正因为误差被压缩在这么小的范围里，13,000 的安全余量才足够「防估算误差」，不需要为了一个可能很大的全局误差去留更夸张的 buffer。
...
还有一种更彻底的做法：调摘要 API 时直接不传 `tools` 参数，模型完全看不到工具定义，自然不会调。GuoLaiCode 走的就是这条路。Claude Code 则保留 `tools` （少一次重新组织 messages 的开销），靠 prompt 把模型「拦」在文本输出。两种思路都对，看实现取舍。
...
[user]      本次会话延续自之前的对话，因上下文空间不足进行了压缩。
            ...
            /home/user/project/.guolaicode/sessions/20250601-143022-a1b2.jsonl
            ---
...
GuoLaiCode 的做法是在 Agent Loop 里捕获这个错误，立刻触发一次 `ForceCompact` ，压缩完成后用新的消息列表 **重试原来的请求&#x20;**。相当于「撞墙了，赶紧压缩，然后再试一次」。如果压缩后仍然超限，就按正常错误流程处理，不再无限重试。
