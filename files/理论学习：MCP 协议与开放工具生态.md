...
上一章给 GuoLaiCode 装上了安全刹车，现在工具可以放心用了。但工具系统本身有一个扩展性瓶颈。

如果用户想让 GuoLaiCode 帮忙查一下 GitHub 上的 Issue 呢？
...
每个需求都是同一个流程：写代码，实现接口，处理第三方 API 的认证和错误格式，注册到 ToolRegistry，重新编译，发布新版本。GuoLaiCode 的代码库越来越胖，但你永远追不上用户的需求。一个人想要 GitHub 集成，另一个想要 GitLab，第三个想要 Bitbucket。你不可能把全世界的 API 都实现一遍。

问题出在哪？

出在 **工具的供给和 Agent 的核心是耦合的&#x20;**。Agent 想用新工具，必须由 Agent 的开发者来写代码、编译、发布。第三方开发者没有办法独立地给 GuoLaiCode 添加工具。

![]()

换个角度想：有没有一种方式，让别人独立开发工具，GuoLaiCode 动态地接入这些工具，不需要改一行代码，不需要重新编译？
...
对 GuoLaiCode 来说，我们只需要实现一次 MCP Client。社区里有人写了 GitHub 的 MCP Server？直接接进来。有人写了数据库的 MCP Server？也直接接进来。不需要改 GuoLaiCode 的任何代码。
...
* **Host&#x20;**：真正的 AI 应用本体。比如 Claude Desktop、Codex、IDE 插件，或者我们正在构建的 GuoLaiCode。

* **Client&#x20;**：Host 里的一个连接组件。它负责和某一个 MCP Server 建立连接、发送请求、接收响应。

* **Server&#x20;**：对外暴露能力的程序。它可能运行在本地，也可能运行在远程。

注意这里有个非常容易混淆的点： **GuoLaiCode 本身是 Host，不是 Client&#x20;**。更准确地说，GuoLaiCode 这个 Host 内部会创建一个或多个 MCP Client，每个 Client 对应一个 Server。
...
这一章我们实现的是 GuoLaiCode 里的 **MCP Client&#x20;**，重点放在两件事：
...
看着眼熟吗？这和我们之前实现的 Tool 接口定义几乎一模一样。名称、描述、参数 Schema，结构完全对得上。唯一的区别是：我们的工具在 GuoLaiCode 进程内执行，MCP 的工具在一个外部进程里执行。
...
stdio 传输是怎么工作的？非常简单：GuoLaiCode 启动一个子进程来运行 MCP Server，然后通过这个子进程的 stdin 和 stdout 管道来通信。GuoLaiCode 往子进程的 stdin 写请求，从子进程的 stdout 读响应。就这样。
...
stdio 传输把这些问题全部消除了。MCP Server 就是一个普通的命令行程序，GuoLaiCode 启动它，通过操作系统的管道通信。不需要额外的网络配置，不需要担心端口占用，不需要服务发现机制。进程启动即可用，退出即清理。进程的生命周期管理是操作系统最擅长的事情。
...
GuoLaiCode（Client）启动 MCP Server 子进程后，第一件事是发送 `initialize` 请求，声明自己的身份和能力。Server 回应自己的身份和能力。就像两个人第一次见面，先自我介绍一下。
...
```plain&#x20;text
{
  ...
  "params": {
    ...
    "capabilities": { "roots": {} },
    "clientInfo": { "name": "GuoLaiCode", "version": "0.1.0" }
  }
}
```
...
拿到工具列表后，GuoLaiCode 就知道这个 Server 有哪些工具可用了。这些工具定义会被包装成 GuoLaiCode 内部的 Tool 接口，注册到 ToolRegistry 里。Agent 在下一轮对话中就能看到它们。
...
## 工具包装器：让 MCP 工具融入 GuoLaiCode

到这里你可能会想：MCP Server 返回的工具定义和 GuoLaiCode 内部的 Tool 接口不是一回事，怎么让 Agent 统一使用？

答案是 **适配器模式&#x20;**。我们写一个 MCPToolWrapper，把 MCP 工具「包装」成 GuoLaiCode 的 Tool 接口。这是一个经典的设计模式：两个接口不兼容，用一个中间层做转换。
...
## 配置：让用户告诉 GuoLaiCode 该连哪些 Server

MCP Server 需要用户来配置。你总不能让 GuoLaiCode 自己去猜用户想连哪些 Server 吧？配置也遵循我们之前建立的多级作用域模式。

项目级配置放在 `.guolaicode.yaml` 里，只在当前项目生效：
...
用户级配置放在 `~/.guolaicode/config.yaml` 里，在所有项目生效：

```plain&#x20;text
# 用户级 ~/.guolaicode/config.yaml
mcp_servers:
  slack:
    command: "npx"
...
注意这里即使某些 MCP Server 连接失败，我们也只是打个警告日志，不阻止 GuoLaiCode 启动。用户配置了 5 个 Server，其中一个因为 Token 过期连不上，不应该影响其他 4 个的正常使用。
...
假设用户配置了 4 个 MCP Server，每个提供 15-20 个工具，加上 GuoLaiCode 自己的 6 个内置工具，工具列表一下子膨胀到 80 个。每个工具的完整 schema 包含名称、描述、参数定义和类型约束，平均占 100 到 300 个 token。80 个工具就是 8,000 到 24,000 个 token 的工具定义，每一轮对话都要带着。还没开始干正事，上下文窗口就被工具定义吃掉了一大块。
...
所以 GuoLaiCode 的默认策略是：第三章的 6 个内置工具始终可见，数量少、使用频率高，没必要延迟。本章引入的所有 MCP 工具全部延迟，因为 MCP 工具数量不可控，用户加一个 Server 就可能多出几十个工具。这个「内置常驻、MCP 延迟」的分界，也是 Claude Code 采用的做法。

***

## 安全考量：信任边界

在本章的 `stdio` 实现里，MCP Server 是外部程序， 运行在 GuoLaiCode 的子进程中 。这里有一个重要的信任边界需要考虑。
...
这一章我们让 GuoLaiCode 从「封闭工具集」变成了「开放工具生态」。

核心思路其实很简单：定义一个标准化的协议（MCP），让 Host、Client、Server 按统一规则协作。Host 是 GuoLaiCode 本体，Client 是它内部负责连接某个 Server 的组件，Server 则提供 Tools、Resources、Prompts 这些能力。Data Layer 负责规定消息语义，Transport Layer 负责规定消息怎么传。
...
MCP 的引入是一个架构上的质变。在此之前，GuoLaiCode 的能力边界由你这个开发者决定，你实现了什么工具，Agent 就能用什么工具。在此之后，能力边界由整个生态决定，任何人都可以开发 MCP Server，GuoLaiCode 都能接入。 从「产品」到「平台」，就差这一步。GuoLaiCode 现在工具够多了，但对话越长，上下文窗口越紧张。下一章讲上下文管理，解决 token 持续增长的问题。
