...
经过前两章的努力，GuoLaiCode 已经是一个挺像样的终端聊天应用了。你可以跟它多轮对话，它的回复还有 Markdown 渲染，看起来挺专业。
...
还记得开篇说的吗？Agent 的四个要素是 LLM + 工具 + 循环 + 反馈。目前我们只有 LLM，没有工具，也就不可能有反馈循环。GuoLaiCode 本质上还是一个聊天机器人。
...
答案是 工具注册中心（Registry） 。它是工具的统一管理入口，把工具的创建和使用解耦。
...
GuoLaiCode 的第一版需要 6 个核心工具。为什么是这 6 个？因为它们覆盖了 Coding Agent 最基本的操作需求：读代码、写代码、改代码、跑命令、找文件、搜内容。缺了任何一个，Agent 的能力都会有明显短板。
...
执行逻辑看起来简单：在文件里找到 old\_ string，替换成 new\_ string，写回文件。但有一个关键约束： **old\_string 必须在文件中唯一匹配&#x20;**。

为什么要这个约束？因为如果 old\_ string 出现了多次（比如模型给了一个太短的字符串 `return` ），你不知道模型想改哪一个。猜错了就是 bug。所以找到多次时直接报错，提示「该字符串出现了 N 次，请提供更多上下文使其唯一」。模型收到这个反馈后，会给出更长、更有区分度的 old\_ string。
...
输出方面，把 stdout 和 stderr 合并到同一个流。如果输出 超过 10000 字符 ，保留前 2000 和后 8000 字符，中间用截断标记连接。为什么保留更多尾部？因为编译错误和测试结果通常在输出末尾。
...
默认情况下，非零退出码标记为 isError: true，但有些命令的退出码 1 不代表出错：grep 退出码 1 表示「没找到匹配」，diff 退出码 1 表示「文件有差异」，find 退出码 1 表示「部分目录不可访问」。这些都是正常的查询结果，不是报错。所以我们维护一个命令语义表，对这类命令只有退出码 2 以上才算 error。其他命令（比如编译器）非零退出码照常标 error，但输出内容里的报错详情本身就是模型修复问题需要的线索。超时是另一种特殊情况，直接返回 isError: true
...
`content_block_start` 告诉你一个新的 tool\_ use 块开始了，给你 id 和 name。然后一系列 `content_block_delta` 给你 JSON 的碎片。最后 `content_block_stop` 告诉你这个块结束了。
...
第一， `tool_result` 是以 user 角色发送的，这是 Claude API 的要求。 上一章留下的那个问题，工具结果到底算谁的消息，到这里就有了答案：它归在 user 名下。所以 user/assistant 交替的格式不用打破，只是这条 user 消息装的是 tool\_result，而不是普通文本。
...
现在 GuoLaiCode 有手有脚了，但每次只能走一步。用户说一句话，模型调一次工具，返回结果，结束。如果一个任务需要先搜索、再读取、再修改、再测试呢？这就需要让工具调用自动循环起来。下一章，Agent Loop。
