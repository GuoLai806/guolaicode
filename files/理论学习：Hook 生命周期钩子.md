...
用了 GuoLaiCode 一段时间之后，你会发现自己养成了一些「习惯」。
...
那这套 YAML 写在哪儿？GuoLaiCode 会在三个地方找 Hook 配置。最常用的是项目根目录下的 `.guolaicode/config.yaml` ，跟着项目走、随仓库一起被人 review。如果你想在多个项目间共享一份配置，可以放到 `~/.guolaicode/config.yaml` ，所有项目都能读到它。还有一个 `.guolaicode/config.local.yaml` ，给你本地的临时改动用，不会被 git 追踪，优先级也最高。三个文件按顺序加载，但和 Skill 那种「找到一个就停」的查找语义不一样：Hook 配置是 **追加合并&#x20;**&#x7684;，用户级和项目级声明的 Hook 都会同时生效，叠加起来用。
...
GuoLaiCode 定义了十几种生命周期事件，覆盖了 Agent 运行的各个阶段。我们按类别来看。
...
最后是 **系统级事件&#x20;**，数量最多。 `startup` 和 `shutdown` 分别在 GuoLaiCode 启动和退出时触发。 `error` 在发生错误时触发。 `compact` 在上下文压缩时触发。
...
条件决定 Hook 在事件触发时是否执行。GuoLaiCode 的条件语法复用了权限规则的匹配语法，你不需要学两套东西。
...
动作是 Hook 触发后执行的操作。GuoLaiCode 支持四种执行器，每种适用于不同的场景。
...
action:
  ...
  method: POST
  body: '{"text": "GuoLaiCode: Agent 修改了 $FILE_PATH"}'
```
...
`once: true` 表示这个 Hook 第一次触发后就标记为「已执行」，后续不再触发。实现上就是一个布尔值： `shouldRun()` 检查 `once && executed` 是否同时为真，是的话就跳过； `markExecuted()` 在执行后把 `executed` 设为 true。重启 GuoLaiCode 会重置这个标记，不做持久化。这是有意的： `once` 的语义本来就是「本次会话只触发一次」，重启 GuoLaiCode 等同于开了个全新会话，配合 `session_start` 这类事件才能正常工作。要做跨进程的「真正只触发一次」，应该用版本检测之类的机制，不是 `once` 该管的事。
...
这一章我们给 GuoLaiCode 装上了 Hook 系统，让它能够在生命周期的关键时刻自动执行动作。
