...
上一章我们给 GuoLaiCode 装了 6 个工具。它能读文件、写文件、搜代码、执行命令。听起来能力已经很全了，对吧？
...
这是 5、6 步操作。但现在的 GuoLaiCode 只能做一步。模型返回一个 `tool_use` ，你执行完返回 `tool_result` ，模型给一个最终回复，结束。它不会自动往下走。
...
**Agent Loop 就是让模型学会「自己干活」的那个机制。&#x20;**&#x5B83;是 GuoLaiCode 从「工具辅助聊天」进化到「自主编程助手」的最后一块拼图。而且说出来你可能不信，它的核心就是一个 while 循环加几次 API 调用。真的就这么简单。
...
用一个具体例子感受一下。假设你让 GuoLaiCode 帮你写一个 HTTP 服务器：
...
所以 GuoLaiCode 的 Agent Loop 采用「事件流」模式：输入是用户消息，输出是一个 AgentEvent 的异步流。
...
有必要。因为把「是否继续」的判断逻辑集中到一个地方，后续扩展会非常自然。随着 GuoLaiCode 的功能越来越复杂，你可能需要加入更多状态。比如 `NEED_CONFIRM` ，遇到破坏性操作时需要用户确认才能继续。或者 `RATE_LIMITED` ，被 API 限流时需要暂停一会儿再重试。如果一开始就用状态机的思维来写，加新状态就是加一个分支的事。如果判断逻辑散落在循环的各个角落，加新状态就变成了到处打补丁。
...
到这里，让我们站高一点，看看 GuoLaiCode 现在的完整数据流：
...
**恭喜。走到这里，GuoLaiCode 已经是一个真正的 Agent 了。**
...
从这一章开始，GuoLaiCode 不再需要你一步步手动催它了。
