...
## 概念有了，但 GuoLaiCode 还不会思考

上一章我们搞清楚了 Agent 的本质和 GuoLaiCode 的架构全景。但到目前为止，一行代码都没写。GuoLaiCode 还只是一张蓝图。
...
GuoLaiCode 同时兼容 Anthropic 和 OpenAI 两套协议，这一章先以 Claude 的 Messages API 为例。OpenAI 协议的适配在封装层处理，后面会讲到。
...
到目前为止你已经见过了 `messages` 数组。但 Claude API 的请求里其实有好几个不同的信息 字段 ，各管各的事：
...
```plain&#x20;text
{
  ...
  "system": "你是 GuoLaiCode，一个终端环境中的 AI 编程助手。\n\n# Environment\n当前工作目录: /home/dev/myproject\n操作系统: Linux\n当前时间: 2026-05-27",
...
到第 20 轮，仅 input\_ tokens 就是 1000 + 20 × (50 + 500) = 12000 tokens。而第 1 轮只有 1050 tokens。差了 10 倍还多。
...
想象一个场景：你花了两周写好了 GuoLaiCode，代码里到处 import 着 Anthropic 的 SDK 类型。有一天老板说，来，换个 GPT 试试。你打开项目一看，消息类型用的是 Anthropic 的、事件解析用的是 Anthropic 的、连错误处理都跟 Anthropic 的 API 绑死了。改一处牵一片，整个项目得翻一遍。
...
前面讲的 LLM 客户端和对话管理器都是引擎层的基础设施，用户看不见也摸不着。最终用户看到的，是一个运行在终端里的交互界面。GuoLaiCode 采用 TUI（Terminal User Interface）方案：上方是对话区域，模型的回复流式显示；下方是输入框，用户在这里输入指令；底部状态栏展示当前模型、Token 用量等信息。下面这张截图就是我们要做成的样子：
...
流式响应是基本要求。SSE 事件序列（message\_ start → content\_ block\_ start → content\_ block\_ delta → content\_ block\_ stop → message\_ delta → message\_ stop）是处理流式响应的基本框架，后面每一章都会用到。
