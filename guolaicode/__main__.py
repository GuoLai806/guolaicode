...
from guolaicode.config import ConfigError, load_config
from guolaicode.hooks import HookConfigError, HookEngine, load_hooks
from guolaicode.permissions import PermissionMode


def main() -> None:
    # 先确保 .guolaicode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".guolaicode").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".guolaicode/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="guolaicode", description="GuoLaiCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    from guolaicode.app import GuoLaiCodeApp
    from guolaicode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    from guolaicode.agent import Agent
    from guolaicode.client import create_client, resolve_context_window
    from guolaicode.conversation import ConversationManager
    from guolaicode.memory.instructions import load_instructions
    ...
    from guolaicode.tools import create_default_registry
