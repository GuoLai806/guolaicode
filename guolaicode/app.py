...
from guolaicode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from guolaicode.commands.completion import CompletionPopup
from guolaicode.commands.handlers import register_all_commands
from guolaicode.config import MCPServerConfig, ProviderConfig
from guolaicode.hooks import HookContext, HookEngine, load_hooks
from guolaicode.conversation import ConversationManager, Message
from guolaicode.mcp import MCPManager
from guolaicode.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from guolaicode.permissions import (
...
from guolaicode.agents.task_manager import TaskManager
from guolaicode.agents.trace import TraceManager
from guolaicode.agents.notification import inject_task_notifications
from guolaicode.commands.handlers.tasks import create_tasks_command
from guolaicode.skills.executor import SkillExecutor
from guolaicode.skills.loader import SkillLoader
from guolaicode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from guolaicode.cache import FileCache
from guolaicode.tools import ToolRegistry, create_default_registry
from guolaicode.tools.agent_tool import AgentTool
from guolaicode.tools.ask_user import AskUserEvent, AskUserTool
from guolaicode.tools.impl.tool_search import ToolSearchTool
from guolaicode.tools.load_skill import LoadSkill
from guolaicode.worktree.cleanup import start_stale_cleanup_task
from guolaicode.worktree.manager import WorktreeManager
from guolaicode.commands.handlers.worktree import create_worktree_command
from guolaicode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".guolaicode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".guolaicode" / "history"
...
class GuoLaiCodeApp(App):
    ...
    def _make_banner(model: str = "", work_dir: str = "") -> RichText:
        t = RichText()
        t.append(" /\\_/\\    ", style="bold color(213)")
        t.append("GuoLaiCode v0.1.0\n", style="color(242)")
        t.append("( o.o )   ", style="bold color(213)")
        t.append(f"{model}\n" if model else "\n", style="color(242)")
        t.append(" > ^ <    ", style="bold color(213)")
    ...
    def on_mount(self) -> None:
        self.register_theme(_GUOLAICODE_THEME)
        self.theme = "guolaicode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".guolaicode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".guolaicode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".guolaicode" / "permissions.local.yaml",
            ),
        ...
        from guolaicode.filehistory import FileHistory
        ...
        from guolaicode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from guolaicode.config import WorktreeConfig
        ...
        from guolaicode.tools.enter_worktree import EnterWorktreeTool
        from guolaicode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from guolaicode.teams.manager import TeamManager
        from guolaicode.tools.team_create import TeamCreateTool
        from guolaicode.tools.team_delete import TeamDeleteTool
        ...
        from guolaicode.commands.handlers.trace import create_trace_command
        ...
        from guolaicode.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from guolaicode.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from guolaicode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from guolaicode.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from guolaicode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from guolaicode.permission_dialog import InlinePermissionWidget
