...
def _session_path(GuoLaiCode_dir: Path) -> Path:
    return GuoLaiCode_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(GuoLaiCode_dir)
...
def load_worktree_session(GuoLaiCode_dir: Path) -> WorktreeSession | None:
    path = _session_path(GuoLaiCode_dir)
    if not path.exists():
        return None
    try:
