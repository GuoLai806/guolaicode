...
from guolaicode.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from guolaicode.worktree.models import Worktree, WorktreeSession
from guolaicode.worktree.session import load_worktree_session, save_worktree_session
from guolaicode.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".guolaicode" / "worktrees"
        )
        self._GuoLaiCode_dir = Path(repo_root) / ".guolaicode"
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._GuoLaiCode_dir)
        if session is None:
            return None
