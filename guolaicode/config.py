...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".guolaicode" / "config.yaml",
        cwd / ".guolaicode" / "config.yaml",
        cwd / ".guolaicode" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .guolaicode/config.yaml "
            "in project or ~/.guolaicode/config.yaml"
        )
    return merged
